...
function init() {
  ...
  $(document).on('click', '.marker-item', function (e) {
    ...
    zoomToMarker($(this).data('x'), $(this).data('y'))
  })
  $(document).on('click', '.marker-del', function (e) {
    e.stopPropagation()
    if (!window.confirm(language.phrases['ui.confirm_delete_marker'] || 'Delete this marker?')) return
    deleteMarker($(this).data('id'))
  })

  // Player list: styled hover tooltip (rank/level + admin attributes)
  $(document).on('mouseenter', '.player-list-item', function () { showPlayerTip(this, $(this).data('cid')) })
  $(document).on('mouseleave', '.player-list-item', hidePlayerTip)
...
function doLogin () {
  ...
  $.ajax({
    ...
    updateAdminUI()
    renderMarkersPanel()
    getPlayers()              // re-fetch so admin-only character stats load
    closeLoginModal()
...
function doLogout () {
  $.ajax({ url: 'api/logout', method: 'POST' }).always(function () {
    ...
    updateAdminUI()
    renderMarkersPanel()
    getPlayers()              // re-fetch to drop admin-only stats
...
function renderCustomMarkers () {
  ...
  customMarkers.forEach(function (m) {
    ...
    mk.on('click', function () { zoomToMarker(m.x, m.y) })
...
function renderPlayerTable () {
  ...
  filtered.forEach(function (player) {
    // Rank/level (+admin attributes) shown in a styled hover tooltip (see player-tip).
    var cid = escapeHtml(String(player.char_id == null ? '' : player.char_id))
    html += '<tr class="player-list-item' + (player.online == 1 ? ' player-online-row' : '') + '" data-cid="' + cid + '">'
...
// ── Player list hover tooltip (styled like the building tooltips) ─────────────
function playerTipHtml (p) {
  var ph = language.phrases
  var badge = (p.online == 1) ? makeBadge('badge-online', '● ' + (ph['ui.online'] || 'Online')) : ''
  var html = '<div class="tip-header">' + escapeHtml(p.char_name || '—') + (badge ? ' ' + badge : '') + '</div>'
  html += tipRow(ph['ui.guild'] || 'Guild', p.guild_name)
  html += tipRow(ph['ui.rank'] || 'Rank', p.rank)
  html += tipRow(ph['ui.level'] || 'Level', p.level)
  if (isAdmin && p.stats) {
    var s = p.stats
    html += '<div class="tip-sep"></div>'
    html += tipRow(ph['ui.attr_str'] || 'Strength', s.str)
    html += tipRow(ph['ui.attr_vit'] || 'Vitality', s.vit)
    html += tipRow(ph['ui.attr_agi'] || 'Agility', s.agi)
    html += tipRow(ph['ui.attr_auth'] || 'Authority', s.auth)
    html += tipRow(ph['ui.attr_grit'] || 'Grit', s.sta)
    html += tipRow(ph['ui.attr_exp'] || 'Expertise', s.mastery)
  }
  return html
}

function playerTipEl () {
  var el = document.getElementById('player-tip')
  if (!el) {
    el = document.createElement('div')
    el.id = 'player-tip'
    el.className = 'side-tip'
    document.body.appendChild(el)
  }
  return el
}

function showPlayerTip (anchor, cid) {
  var p = playersData.find(function (x) { return String(x.char_id) === String(cid) })
  if (!p) return
  var el = playerTipEl()
  el.innerHTML = playerTipHtml(p)
  el.style.display = 'block'
...
// Center on a point and set the zoom level to 2.
function zoomToMarker (x, y) {
  map.setView(toLatLng(x, y), 4)
}
