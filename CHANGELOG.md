...
- **Character attributes (admin)** — hovering a player in the list shows their six attributes (Strength / Vitality / Agility / Authority / Grit / Expertise); sent only to logged-in admins
- **Right-click context menu** — "Copy coordinates" (X Y) and "Add marker" (admin)
- Muted the per-clan marker palette so decay warnings stand out; modals close only via Esc / ✕ (not outside click); panels switch only manually (no longer reset on map click); clicking a marker centres and zooms to it; marker deletion asks for confirmation
